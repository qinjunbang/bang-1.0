describe 'Morris.Donut', ->
  ...
  describe 'svg attributes', ->
    ...
    it 'should have a label with fonts size 15', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[fonts-size='15px']").size().should.equal 1

    it 'should have a label with fonts size 14', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[fonts-size='14px']").size().should.equal 1

    it 'should have a label with fonts-weight 800', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[fonts-weight='800']").size().should.equal 1
